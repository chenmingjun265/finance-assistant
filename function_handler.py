...
async def LLM_replay(prompt_template,messages):
    ...
    message:大模型调用的用户输入
    """
    llm_client =AsyncOpenAI(api_key=os.getenv("API_KEY"), base_url="https://dashscope.aliyuncs.com/compatible-mode/v1")  # API Key 仅从 .env 环境变量读取
...
stock_data = pd.read_excel(r'./stock_data.xlsx', parse_dates=['Date'],sheet_name='中国银行')
# 确保日期为索引
stock_data.asfreq('D')
np.random.seed(42)
print(stock_data)
