"""
金融问题咨询MCP,包含2个工具:
(1) 金融问题本地RAG回复（优先本地服务，较为专业，避免幻觉）
...
"""
import sys
sys.path.append(r"./project2_Agent_SDK+MCP_materials")
...
# def init_vector_db():
policy_info_data=pd.read_excel(r"./投资政策.xlsx")
print(policy_info_data)
#以上的三行有另外一种写法，上边那种写法必须在我们已经cd进本目录的情况下才适用，属于是相对路径
#而下边这种写法更适合代码迁移使用，采用的是绝对路径的读取方法，基本上所有的文件都可以采用这种方法
#使用的时候只需要把"投资政策.xlsx"改成其他的变量名或者其他的具体文件名就可以了
#def init_vector_db():
#    base_dir = os.path.dirname(os.path.abspath(__file__))
#    full_file_path = os.path.join(base_dir, "投资政策.xlsx")
#    policy_info_data=pd.read_excel(full_file_path)
#    print(policy_info_data)
...
class QwenEmbeddingFunction:
    def __init__(self, api_key: str =os.getenv("API_KEY"), model_name: str = "qwen3.7-text-embedding"):
        """
        初始化通义千问Embedding函数
        参数:api_key: 阿里云API密钥
    ...
    def name(self) -> str:
        # chromadb 1.0+强制要求这个方法
        return "qwen-custom-embedding"
...
class ChromaDBWithQwen:
    def __init__(self, collection_name: str = "policy_collection", persist_directory: str = None):
        ...
        """
        # 创建自定义的embedding函数
        self.embedding_function = QwenEmbeddingFunction(api_key=os.getenv("API_KEY"), model_name="qwen3.7-text-embedding")
        print(self.embedding_function)
        # 初始化Chroma客户端
        self.client = chromadb.PersistentClient(path=persist_directory) #path='./'
        #上边这行的path=persist_directory意思是可以实例化地保存在我们的本地中，也就是后边的db_data的来源
        # 创建一个可将数据持久化存储到磁盘的客户端。该客户端适用于本地开发与测试环境
        #。生产环境请优先使用服务端部署的 Chroma 实例
        # 获取或创建集合
...
qwen_db=ChromaDBWithQwen(persist_directory=r"./db_data")
print(qwen_db)
#%%
