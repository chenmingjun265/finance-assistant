...
# Gradio checks its own localhost endpoint during launch.  Keep that request
# away from system HTTP proxies, which otherwise may return a 502 response.
_local_hosts = "localhost,127.0.0.1,0.0.0.0"
for _proxy_bypass_var in ("NO_PROXY", "no_proxy"):
    _existing_bypass = os.environ.get(_proxy_bypass_var, "")
    os.environ[_proxy_bypass_var] = ",".join(
        item for item in (_existing_bypass, _local_hosts) if item
    )
...
if __name__ == "__main__":
    ...
    demo.launch(
        ...
        share=False, # 如需 Gradio 临时公网链接，可手动改为 True
    )
