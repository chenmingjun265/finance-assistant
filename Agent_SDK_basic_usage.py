...
2、这是一个事件驱动的多智能体框架，每次运行产生一系列的时间通过.new_items属性查看历史所有事件
...
import sys
sys.path.append(r'./project2_Agent_SDK+MCP_material')
...
external_client = AsyncOpenAI(
    ...
    api_key=os.getenv("DEEPSEEK_API_KEY")) #DEEPSEEK_API_KEY="" os.getenv("DEEPSEEK_API_KEY"))
...
consult_agent = Agent(name="consult agent",  # 智能体名称：咨询智能体
              instructions="你是一个只能回答金融领域的智能体", #智能体的功能描述,system_prompt 你是一个善于回答金融问题的助理
...
def predict_future_7data(stock_name):
    if stock_name=="上证指数":
        stock_data = pd.read_excel(r'./stock_data.xlsx',
...
poem_agent = Agent(
    ...
    handoff_description="当用户输入描述美景的话时，调用该智能体回答问题",
    #告诉LLM什么时候转交给该智能体，这是对智能体转接逻辑的极大强化
    model=deepseek_model
)
#粗俗的智能体
vulgar_agent = Agent(
...
async def main6():
    response = await Runner.run(switch_agent, input="今天月亮好圆啊你个垃圾") #今天月亮好圆啊
    print(response.final_output)
