...
#{session_id: {"messages": List, "current_agent": Agent,"user_id":"user1"}}这就是上边的global_sessions的格式
class MCPManager:
    """管理所有MCP服务器的连接"""
    def __init__(self):
        self.servers = {}
    ...
    async def check_connections(self):
        """检查服务器连接状态，必要时重新连接"""
        #sse模式很容易在规定时间内连接超时如54行：client_session_timeout_seconds=200
        #所以需要对服务器的连接状态进行检查
        for name, server in self.servers.items():
            try:
                # 尝试简单操作来检测连接状态
...
class FinancialAssistant:
    def __init__(self):
        ...
        # asyncio.Lock() 共享锁：确保初始化代码只执行一次，就是为了避免多次调用导致多个实例化
        # 每次调用都执行一个实例化mcp，跟之前的那个db_data的思维是一样的，只需要一个db就够了
        # 即使多个client同时调用我这个mcp我这个 initialize()实例化依旧能够确保是全局唯一的
    ...
    async def _initialize_agents(self) -> Agent:
        ...
        # 设置智能体之间的互转关系，通过handoffs.extend进行智能体之间关系的链接
    ...
    async def create_session(self, user_id: str = "default_user") -> str:
        ...
        #随机创建一个id，确保不重复，这个session_id就能传到第26行的global_sessions里边了
    ...
    async def process_message(self, session_id: str, message: str) -> str:
        ...
        for new_item in result.new_items:
            ...
            if isinstance(new_item, MessageOutputItem):
                ...
                response += f"{agent_name}: 工具调用结果: {str(new_item.output)[:100]}...\n"
